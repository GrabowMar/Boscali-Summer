...
- **Dynamic trenches (experimental)** — a trench line is a Bezier curve fitted to the real
  ...
  rise instead of marching along a grid edge. Each ~2.4km position is a field-scale earthwork
  — a walkable ditch inside a broad parapet, spoil and skirt footprint, with a procedural wire
  belt in front of it — that matures from a scrape into a deliberate defensive belt (support
  ...
  behind the parados as shelters, with four native MG/ATGM/MANPADS emplacements per position:
  sparse, interlocking, and lethal. The carved ditch follows the terrain on both sides of its
...
- **Radio** — one local map-MFD screen (`RAD`) with two pages, routed through the game's music
  mixer. **RECEIVER** is the set: a spectrum waterfall, an S-meter fed by a real link budget
  (range, radio horizon and terrain line of sight to each built-in station's tower), TUNE /
  SEEK / SCAN on FM (100 kHz), VHF air (25 kHz, AM) and MW (10 kHz), squelch with SQL keys,
  wide/narrow bandwidth, a MODE override that garbles the wrong demodulator, fine tuning, a
  LINK block and an AF volume row. It behaves like a radio — you tune a station and hear its
  programme; you do not pick tracks. A built-in whose tower is lost (an airbase falls, an HQ
  dies) reads **off air**. **DECK** is the local player: browse your library folders, play,
  pause, skip, shuffle, repeat. While either page is live the game's soundtrack stays held
  (including dead air between stations) and comes back on STOP. Carrier static, squelch and
  the morse station ident are synthesized in memory; transmit, crypto and the peer net are
  honest placeholders, and the deck ducks under a received transmission once voice exists.
  Three starter stations; no bundled audio.
- **Squad & aces** — `SQD` holds **PILOT**, **SKILLS**, **WINGS** and **STUDIO**. **PILOT**
  ...
  Each is five grades, picked one per grade as score accrues; grade 1 is the tool and each
  later grade needs the one before it. A career holds **two** tools, so two lanes open and
  two stay closed — mixing is allowed, depth is what the pick budget buys, and grade 5 is
...
- **Weather** (`Weather.Enabled`, default on) — the host drives the mission's sky from a
  deterministic front schedule: cloud coverage, cloud base, wind speed/heading and
  turbulence ramp between fronts on the mission clock, and every peer derives the same
  forecast from that clock alone, so a late joiner is immediately correct and no new weather
  message is added — vanilla's own sync vars carry the values. The same schedule makes
  weather a place: up to three storm cells advect with the front wind, derived identically
  on every peer from the seed, clock, map size and wind — none of it is transmitted — and
  the supercell cloud towers, falling rain, canopy streaks, cockpit weather HUD and `WEA`
  radar scope all read that one field, each with its own client-local setting
  (`RainEffects`, `RainOnCanopy`, `RainAudio`, `RainEffectDensity`, `Hud`, `Supercells`,
  `SupercellDetail`, `RadarRangeKm`). `CloudSortFudge` biases a cell against the vanilla
  cloud deck, and `ReplaceVanillaClouds` (off by default) takes the local deck away so the
  cells own the sky while vanilla keeps driving the cloud cookies, occlusion, fog and the
  distant horizon band. The `WEA` environment panel shows the current sky and the forecast;
  an opt-in debug overlay (`Weather.DebugControls`, `F11` + Ctrl) aims it directly on the
  host. Authored `ModifyEnvironment` mission beats always win — a change the model did not
  make is adopted and held for 40 seconds before the schedule resumes — and ground fires
  thicken the sky through the existing fire read. In-game acceptance is pending: the panel
  render, the storm field and its supercell, rain, HUD and radar presentation, the ramp in a
  live mission, the fire-haze read, client display, the sort bias and both cloud-hijack
...
4. Maximise the tactical map. Press **`RAD`** for the receiver page — TUNE/SEEK/SCAN work the
   band, SQL sets the squelch, BAND cycles FM / VHF air / MW — and its **DECK** tab for your
   own folders and transport (**FOLDER** adds OGG/WAV stations). Press **`SQD`** for
