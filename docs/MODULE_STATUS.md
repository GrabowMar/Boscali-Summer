...
- The mod-drawn markers and the vicinity card are new and unverified in game: check the cockpit
  ...
  ring at the same radius, the second line and the `#N` prefix in the vanilla font, three
  contracts in one direction nudging apart instead of stacking, the map icons at 20/40 px with
  the duplicate mask at minimum and maximum zoom, `MapOptions.showObjectives` off, the card with
  an inside contract / a lost contact / three contracts at once, and the enter and leave
  banners. Also confirm the log's one-line warning appears (and markers stay hidden) if
