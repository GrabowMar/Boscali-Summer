...
{
    /// <summary>
    /// While a full-screen OPS overlay (station uplink, CYBER console) covers the screen, the
    /// maximised map under it must not pan or zoom. <c>MapControls</c> reads the mouse through Rewired, not uGUI, so the uplink's
    ...
    {
        private static bool Prefix() => !Presentation.FullscreenInput.AnyOpen;
    }

    /// <summary>
    /// A cursor over a full-screen OPS overlay is not a map position: armed call-ins and other modules' map
    ...
    internal static class UplinkMapCursorGuardPatch
    {
        ...
        {
            if (!Presentation.FullscreenInput.AnyOpen) return true;
            __result = false;
            return false;
        }
    }
